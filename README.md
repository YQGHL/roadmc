...
| 当前基线 v2 | ✅ Disease IoU `0.4781` [0.4435, 0.5105]（独立 test，阈值 0.320，ECE 0.0028）；同协议 30 epochs 重跑 `0.5483` [0.5126, 0.5820]（阈值 0.370，ECE 0.0069） |
| 当前模型 | `Swin3D + DSCM + Muon/AdamW` |
| R2 mixing 消融 | ✅ 三种子完成（42--44）：`none` `0.5000±0.0103` / `mHC-2` `0.5100±0.0126`（20ep）——单种子 +0.026 的优势**未跨种子复现**，不作机制性结论 |
...
### 基线 v2 · 30 epochs 同协议重跑（论文口径）

同协议、仅延长训练轮数（15 → 30 epochs，其余超参不变；cosine 的 `T_max` 随 `max_epochs` 拉伸）：

| 指标 | 15 epochs | 30 epochs |
| --- | --- | --- |
| val IoU（阈值冻结后评估） | 0.4608 | 0.5219 [0.4811, 0.5604] |
| **test IoU** | **0.4781** [0.4435, 0.5105] | **0.5483** [0.5126, 0.5820] |
| 冻结二分类阈值 | 0.320 | 0.370 |
| Precision / Recall（前景） | 0.7092 / 0.5947 | 0.8091 / 0.6298 |
| ECE / Brier / NLL | 0.0028 / 0.1061 / 0.1927 | 0.0069 / 0.0912 / 0.1713 |

**结论**：30 epochs 相对 15 epochs 提升 `+0.070` 绝对（`+14.7%`），印证 15-epoch 锚点在
训练末仍未收敛（`TECHNICAL_REPORT_v2.md` §3 的原有判断）。两批同协议可直接比较；
该提升仍是合成域、二分类、单种子结果，不外推到真实道路或 38 类。
...
> **结论强度红线（2026-09-12 更新，三种子已完成）**：`none` `0.5000±0.0103` 对
> `mHC-2` `0.5100±0.0126`（seed 42/43/44，样本标准差）——均值差 `+0.010` 小于组内 std，
> 且 seed 44 出现反转（0.4959 对 0.5077）→ **单种子 `hc2` 领先（+0.026）未跨种子复现**，
> 不作机制性结论；论文按"方向一致、未达显著"呈报。`dscm`/`hc4` 仍为 seed 42 单种子
> 参考（`dscm` 0.4817 与 `none` 0.4883 同量级）。
